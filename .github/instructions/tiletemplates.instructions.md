...
- Name new multi-tile assets with an unambiguous terminal footprint suffix such as `_2x2` or `_3x3`. Every building has one midpoint `zn_tile_direction` entity whose `direction` names its authored-local entrance edge. Add zero or more `zn_road_connection` entities only at exterior footprint segments requiring physical road branches; every `t_junction` marker targets one adjacent ordinary straight-road tile. Motorways and concrete paths are valid frontage but are never replaced by a requested T-junction.
