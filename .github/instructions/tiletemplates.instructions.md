...
- Follow the canonical orientations in [docs.md](../../docs.md): roads/motorways run south-to-north at $0^\circ$, buildings expose their entrance on the south edge, carpark entrances connect on the south edge, and border assets have their documented edge orientation.
...
- Name new multi-tile assets with an unambiguous terminal footprint suffix such as `_2x2` or `_3x3`. Every building has one midpoint `zn_tile_direction` entity whose `direction` names its local tile edge; the building is rotated so that edge aligns with an adjacent road, motorway, or concrete path, if one is present. Add zero or more `zn_road_connection` entities only at exterior footprint segments requiring physical road branches; every `t_junction`, `bus`, or `driveway` marker targets one adjacent ordinary straight-road tile and replaces it with the matching `roads/tile_road_tjunction.vmf`, `roads/tile_road_bus.vmf`, or `roads/tile_road_driveway.vmf`. Motorways and concrete paths are valid frontage but are never replaced by a requested connection.
...
4. After a change touching border walls, border water, or road-topology variety, run `./bin/test_border_showcase.ps1`; it plans and builds `bin/border_showcase_fixture_map.json` (a small square grid with a fixed seed that deterministically triggers the NW water corner) into `generated/border_showcase` and asserts the water-corner/tip/pier pieces, wall-variation pool, and topology variety.